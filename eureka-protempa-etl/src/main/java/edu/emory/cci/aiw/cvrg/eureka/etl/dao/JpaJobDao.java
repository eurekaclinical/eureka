...
public class JpaJobDao implements JobDao {
	...
	public Job get (Long id) {
		...
		Query query = this.entityManager.createQuery ("select j from Job j where j.id = ?1" , Job.class).setParameter(1 , id);
    ...
    public List<Job> get (JobFilter jobFilter) {
		...
		Query query = this.entityManager.createQuery ("select j from Job j" , Job.class);
        @SuppressWarnings ("unchecked")
        List<Job> resultList = query.getResultList();
