...
public class XlsxDataProvider implements DataProvider {
	...
	public XlsxDataProvider(FileUpload inDataFile, Locale locale) throws DataProviderException {
		...
		if (inDataFile.getLocation() == null) {
	...
	public List<Enrollment> getEnrollments() throws DataProviderException {
		if (this.enrollments == null) {
			this.enrollments = this.readEnrollments();
		}
	...
	public List<MedicationDispense> getMedicationDispenses() throws
		...
		if (this.dispenses == null) {
			this.dispenses = this.readMedicationDispenses();
		}
	...
	private List<Enrollment> readEnrollments() throws DataProviderException {
		XSSFSheet sheet = readRequiredSheet("enrollment");
		String sheetName = sheet.getSheetName();
		List<Enrollment> result = new ArrayList<Enrollment>();
		Iterator<Row> rows = sheet.rowIterator();
		LOGGER.debug("Enrollment iterator: {}", Integer.valueOf(rows.hashCode()));
		rows.next(); // skip header row
		while (rows.hasNext()) {
			Row row = rows.next();
			Enrollment enrollment = new Enrollment();
			enrollment.setId(readLongValue(sheetName, row.getCell(0)));
			enrollment.setPatientId(readLongValue(sheetName, row.getCell(1)));
			enrollment.setStart(readDateValue(sheetName, row.getCell(2)));
			enrollment.setEnd(readDateValue(sheetName, row.getCell(3)));
	...
	private List<MedicationDispense> readMedicationDispenses() throws
			DataProviderException {
		XSSFSheet sheet = this.workbook.getSheet("eDISPENSE");
		List<MedicationDispense> result = new ArrayList<MedicationDispense>();
		if (sheet != null) {
			String sheetName = sheet.getSheetName();
			Iterator<Row> rows = sheet.rowIterator();
			rows.next(); // skip header row
			while (rows.hasNext()) {
				...
				dispense.setId(readStringValue(sheetName, row.getCell(0)));
				dispense.setEncounterId(readLongValue(sheetName,
						row.getCell(1)));
				dispense.setTimestamp(readDateValue(sheetName,
						row.getCell(2)));
				dispense.setEntityId(readStringValue(sheetName,
						row.getCell(3)));
				result.add(dispense);
			}
		}
